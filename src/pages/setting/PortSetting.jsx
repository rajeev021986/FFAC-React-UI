...
const PortSetting = () => {
    ...
    const { data, isLoading, error: geterror, refetch } = useGetOptionsSettingsQuery("port_settings");
    ...
    const Postdata = async () => {
        ...
        setIsLoading(true);
        await addOptons({ body: { port_settings: filteredData }, type: "port_settings" }).then((res) => { if (res.error) { toast.error(res.error.data.error) } else { toast.success(`setting Updated Successufully`) } }).catch(() => console.log("filteredData"))
        refetch();
        setIsLoading(false);
    };
    ...
    return (
        <div style={{ padding: "1rem" }}>
            <Grid xs={12} sx={{ marginBottom: "10px" }} ><Typography variant="h4">Port Setting</Typography></Grid>
