...
const VesselVoyageSetting = () => {
  ...
  const {
    ...
    error: geterror,
    refetch,
  } = useGetOptionsSettingsQuery("vessel_voyage_settings");
  ...
  useEffect(() => {
    ...
    setPortOperator(data?.body.portOperator || []);
  }, [data, geterror]);
  ...
  const Postdata = async () => {
    const filteredData = {
      ...
      portOperator: portOperator.filter(
        (item) => !item.value.includes("Type the")
      ),
    };
    setIsLoading(true);
    await addOptons({
      body: { vessel_voyage_settings: filteredData },
      type: "vessel_voyage_settings",
    })
      .then((res) => {
        if (res.error) {
  ...
  return (
    <div style={{ padding: "1rem" }}>
      <Grid xs={12} sx={{ marginBottom: "10px" }}>
        <Typography variant="h4">Vessel Voyage Setting</Typography>
      </Grid>
