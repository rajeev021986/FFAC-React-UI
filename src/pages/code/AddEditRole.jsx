...
export default function AddEditRole() {
  ...
  useEffect(() => {
    const getRolePermissions = async () => {
      await ApiManager.getRolepermissions(id)
        .then((res) => {
  ...
  const renderPermissionSwitch = (permission) => (_, record) =>
    (
      <Switch
        ...
        onChange={(e) =>
          handlePermissionChange(e.target.checked, record.subMenuId, permission)
        }
      />
    );
  const columns = [
      ...
      render: (_, record) => {
        ...
        return (
          <Switch
            ...
            onChange={(e) =>
              handleSelectAllChange(e.target.checked, record.subMenuId)
            }
          />
        );
      },
  ...
  const handleCheckboxChange = (menuId, menuName, parentId) => {
    ...
    if (isInTable) {
      ...
      if (parentId) {
        ...
        if (
          data.body.find((menu) => menu.menuId === menuId).nestedMenuEntities
            .length <= 0
        ) {
          const newTableItem = {
            subMenuId: menuId,
        ...
        const newTableItem = data.body
          .find((menu) => menu.menuId === menuId)
          .nestedMenuEntities.map((item) => {
            return {
          ...
          .filter(
            (items) =>
              !tableData.find((item) => item.subMenuId === items.subMenuId)
          );
        if (newTableItem.length <= 0) {
          let menuIds = tableData
            .filter((menu) => menu.parentId === menuId)
            .map((menu) => menu.subMenuId);
          setTableData((prev) => [
            ...prev.filter((item) => !menuIds.includes(item.subMenuId)),
          ]);
          setCheckedItems((prev) => {
            const newValues = { ...prev };
            delete newValues[menuId];
          ...
          setAssigned((prev) =>
            prev.filter((item) => item.parentId !== menuId)
          );
          return;
        }
        setTableData((prev) => [...prev, ...newTableItem]);
        setCheckedItems((prev) => {
          return {
            ...
            ...Object.assign(
              {},
              ...newTableItem.map((item) => ({ [item.subMenuId]: true }))
            ),
  ...
  const handleAddRole = async () => {
    const formattedData = {
      ...
      menuPermissions: tableData
        ...
        .reduce((acc, curr) => {
          ...
          let parentMenu = data?.body.find(
            (menu) => menu.menuId === curr.parentId
          );
          if (acc.find((menu) => menu.menuId === curr.parentId)) {
            let preAcc = acc.map((menu) =>
              menu.menuId === curr.parentId
                ? { ...menu, nestedMenus: [...menu.nestedMenus, curr] }
                : menu
            );
            return preAcc;
          } else {
    ...
    if (id) {
      await ApiManager.updateRolePermissions(formattedData)
        .then((res) => {
      ...
      const response = await ApiManager.addRole(formattedData)
        .then((res) => {
          toast.custom(<CustomToast message={res.message} toast="success" />, {
            closeButton: false,
          });
  ...
  return (
    <div
      style={{ display: "flex", gap: "20px", width: "100%", flexWrap: "wrap" }}
    >
      <div
        style={{
          padding: "20px",
              ...
              >
                ...
                >
                  <Checkbox
                    edge="start"
                    checked={
                      checkedItems[menu.menuId] ||
                      (tableData.filter(
                        (items) => items.parentId === menu.menuId
                      ).length ==
                        data.body.find((menus) => menus.menuId === menu.menuId)
                          .nestedMenuEntities.length &&
                        data.body.find((menus) => menus.menuId === menu.menuId)
                          .nestedMenuEntities.length > 0) ||
                      false
                    }
                    onChange={() =>
                      handleCheckboxChange(menu.menuId, menu.menuName, null)
                    }
                    size="small"
