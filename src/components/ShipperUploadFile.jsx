...
import { useAddAgentMutation, useDownloadDocumnentMutation, useGetCustomerFileListMutation, useUploadShipperFileMutation } from "../store/api/shipperDataApi";
...
const ShipperUploadFile = ({ shipper_id,source, disabled, dropdownData}) => {
  // const [uploadCustomerFile, { isLoading }] = useUploadCustomerFileMutation();
  const [uploadShipperFile] = useUploadShipperFileMutation();
  ...
  const handleDialogSave = async () => {
    ...
    try {
      setLoading(true);
      let response = await uploadShipperFile(uploadData).unwrap();
      setDialogOpen(false);
      reloadDataHandler();
      setLoading(false);
    } catch (error) {
  ...
  const reloadDataHandler = async () => {
    try {
      let source = "SHIPPER";
      setLoading(true);
      const res = await ApiManager.getShipperFormData(source, shipper_id);
...
export default ShipperUploadFile;
