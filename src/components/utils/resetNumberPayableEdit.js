...
const ResetNumberPayableEdit = ({ id, value, field, api, row }) => {
  ...
  const handleChange = async (e) => {
    ...
    const invoiceType = row?.invoiceType || "PAY";
    const invoiceCode = invoiceType
      .replace(/[^a-zA-Z]/g, "")
      .substring(0, 3)
      .toUpperCase();

    const newPattern = generatePattern({
      invoiceType: invoiceCode,
    ...
    await api.setEditCellValue({ id, field: "invoicePattern", value: newPattern });
    await api.setEditCellValue({ id, field: "sampleInvoiceNumber", value: newSample });
...
export default ResetNumberPayableEdit;
